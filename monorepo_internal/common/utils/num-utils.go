...
func StrToInt8Def(input string, def int8) int8 {
	res, err := StrToInt8(input)
	if err != nil {
		return def
	}
	return *res
...
func StrToInt32Def(input string, def int32) int32 {
	res, err := StrToInt32(input)
	if err != nil {
		return def
	}
	return *res
...
func StrToFloat32Def(input string, def float32) float32 {
	sourceNumVal, error := strconv.ParseFloat(strings.Trim(input, " "), 32)
	if error != nil {
		return def
	}
	return float32(sourceNumVal)
}
