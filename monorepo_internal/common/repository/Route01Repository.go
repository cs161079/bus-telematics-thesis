...
type Route01Repository interface {
	...
	Delete() error
	AllRecords(context.Context) (*sql.Rows, error)
}
...
func (r route01Repository) AllRecords(ctx context.Context) (*sql.Rows, error) {
	slqDb, err := r.DB.DB()
	if err != nil {
		return nil, err
	}
	rows, err := slqDb.QueryContext(ctx,
		`SELECT rt_code, routed_x, routed_y, routed_order
		   FROM route01
		  ORDER BY rt_code, routed_order`)
	if err != nil {
		return nil, err
	}
	return rows, nil
