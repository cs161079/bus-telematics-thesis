...
type BusRepository interface {
	...
	SelectCapacity(int64, int32) (*models.Bus_Capacity, error)
...
func (r busRepository) SelectCapacity(busId int64, routeCode int32) (*models.Bus_Capacity, error) {
	var result models.Bus_Capacity
	dbResult := r.DB.Table(db.BUSCAPACITY).Where("bus_id=? AND route_id=?", busId, routeCode).
		Order("date_time desc").Limit(1).Find(&result)
