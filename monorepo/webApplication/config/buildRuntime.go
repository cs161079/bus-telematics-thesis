...
func NewApp(db *gorm.DB, lineCtrl,
	extCtrl controllers.ExtApiController, mobileCtrl controllers.MobileController,
	adminCtrl controllers.AdminController,
	promitheus controllers.PrometheusController) *App {
	...
	mainGrp := eng.Group("/api/v1")

	// Εδώ είναι ο Controller για τον Promhtheus Client
	promitheus.AddRouters(mainGrp)
	// Εδώ είναι ο Controller για την Εφαρμογή των κινητών
	mobileCtrl.AddRouters(mainGrp)
	// Εδώ είναι ο Controller για την εφαρμογή Admin Portal
	adminCtrl.AddRouters(mainGrp)
	// Εδώ είναι ο Controller για την εγαρμογή στο Raspberry Pi
	extCtrl.AddRouters(mainGrp)
...
func BuildInRuntime() (*App, error) {
	...
	servicesConstructors := []interface{}{
		...
		controllers.NewMobileController,
		NewApp,
	}
