...
type App struct {
	...
	otpService  TripPlannerService
}

func NewApp(db *gorm.DB, logger logger.OpswLogger, syncSrv SyncService, otpSrv TripPlannerService) *App {
...
func (a App) Boot() {
	...
	if err := a.otpService.PeparationProcess(); err != nil {
		a.logger.ERROR(fmt.Sprintf("Σφάλμα κατά την δημιουργία αρχείων GTFS. %s\n", err.Error()))
	}
