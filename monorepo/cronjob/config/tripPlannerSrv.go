...
import (
	...
	ziputil "github.com/cs161079/monorepo/cronjob/utils"
)

type TripPlannerService interface {
	IntializeService()
	PeparationProcess() error
	agencyData() error
	stopsData() error
	routesData() error
	calendarData() error
...
func NewOtpService(stopSrv service.StopService, routeSrv service.RouteService,
...
func (s *tripPlannerServiceImp) PeparationProcess() error {
	start := time.Now()
	s.IntializeService()
	...
	logger.INFO("Prepare GTFS Data for Shape Data.")
	err = s.shapeData()
	if err != nil {
		return err
	}
	...
	gtfs_dest := os.Getenv("gtfs.location")
	if gtfs_dest == "" {
		gtfs_dest = "gtfs_feed.zip"
	}

	logger.INFO(fmt.Sprintf("Prepare GTFS Feed ZIP file to store %s for OTP.", gtfs_dest))
	err = ziputil.Zip(
		[]string{"gtfs/agency.txt", "gtfs/calendar.txt", "gtfs/routes.txt", "gtfs/shapes.txt", "gtfs/stop_times.txt",
			"gtfs/stops.txt", "gtfs/trips.txt"},
		gtfs_dest,
	)
...
func (s *tripPlannerServiceImp) tripsData() error {
	...
	writeLine(file, "route_id,service_id,trip_id,trip_headsign,shape_id")
...
func (s *tripPlannerServiceImp) shapeData() error {
	fileName := "shapes.txt"
	fullPath := filepath.Join(s.gtfsFolder, fileName)
	...
	rows, err := s.routeSrv.AllRoute01(ctx)
	if err != nil {
		return err
	}
	defer rows.Close()

	_, err = file.WriteString("shape_id,shape_pt_lat,shape_pt_lon,shape_pt_sequence\n")
	if err != nil {
		return err
	}
	for rows.Next() {
		...
		if err := rows.Scan(&route_code, &route_x, &route_y, &route_order); err != nil {
			return err
		}
		_, err := file.WriteString(fmt.Sprintf("S_%d,%f,%f,%d\n", route_code, route_y, route_x, route_order))
		if err != nil {
			return err
		}
	}
...
func (s tripPlannerServiceImp) writeTripsFile(file *os.File, rec TripGTFS) {
	...
	// }
	writeLine(file, fmt.Sprintf("%d, %s, %s, %s, S_%d", rec.RouteId, rec.ServiceId, rec.TripId, rec.TripHead, rec.RouteId))
}
