...
func NewOasaService(routeRepo repository.RouteRepository, mapper mapper.OasaMapper,
	rest RestService, busSrv BusService) OasaService {
...
func (c OasaServiceImpl) GetBusArrival(stop_code int32) ([]models.StopArrival, error) {
	...
	for i, rec := range extraInfo {
		...
		busCapacity, err := c.busSrv.GetBusCapacity(int64(rec.Veh_code), rec.Route_code)
		if err != nil {
			logger.ERROR(err.Error())
		}
		rec.Capacity = int(busCapacity.Bus_Cap)
		rec.Passengers = int(busCapacity.Bus_Pass)
	}
